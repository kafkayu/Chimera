...
# 2. Modifying attention mask for integration with Chimera
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        # [MODIFIED] add chimera mask
        if hasattr(self, "chimera_mask") and self.chimera_mask is not None:
            chimera_mask = self.chimera_mask
            chimera_len = chimera_mask.size(-1)
            combined_attention_mask[:, :, -chimera_len:, -chimera_len:][
                chimera_mask == 0
            ] = combined_attention_mask.min()
            if hasattr(self, "chimera_mode"):
                # debug mode
                if self.chimera_mode == "debug":
                    torch.save(combined_attention_mask, "chimera_mask.pt")
