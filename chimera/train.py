...
>>>>>>> fa0a01236fdadf41f327b08427e8eedcfd403349
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if hasattr(model, "module"):
            chimera = model.module.chimera_heads
        else:
            chimera = model.chimera_heads
        ...
        for i in range(chimera):
            ...
            for k in range(1, 6):
                ...
                topk = topk[not_ignore]
                correct = topk.eq(chimera_labels.unsqueeze(-1)).any(-1)
                log[f"chimera{i}_model_top{k}"] = correct.float().mean().item()
            log[f"chimera{i}_model_loss"] = loss_i.item()
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    chimera_num_heads: int = field(
        default=1,
        metadata={"help": "Number of Chimera heads."},
    )
    chimera_num_layers: int = field(
        default=1,
        metadata={"help": "Number of layers for each Chimera head."},
    )
...
>>>>>>> fa0a01236fdadf41f327b08427e8eedcfd403349
    ...
    # Add Chimera heads
    chimera_lm_head = ChimeraModel(
        model,
        chimera_num_heads=training_args.chimera_num_heads,
...
>>>>>>> fa0a01236fdadf41f327b08427e8eedcfd403349
    ...
    # Generate Chimera config for pushing to HF hub
...
>>>>>>> fa0a01236fdadf41f327b08427e8eedcfd403349
    ...
    # Save Chimera config
    chimera_config.save_pretrained(training_args.output_dir)

    # import pdb; pdb.set_trace()
    # Start trainner
    trainer = CustomizedTrainer(
        model=chimera_lm_head, tokenizer=tokenizer, args=training_args,compute_metrics=compute_metrics, **data_module
    )
